...
class ClearanceAPIServer:
    ...
    def __init__(self, headless: bool, thread: int, page_count: int,
        ...
        self.app.get("/turnstile/sync")(self.process_turnstile_sync)
        self.app.get("/clearance")(self.process_clearance)
        self.app.get("/result")(self.get_result)
        self.app.get("/health")(self.health)
        self.app.head("/health")(self.health)
        self.app.get("/debug/screenshot")(self.get_debug_screenshot)
    ...
    async def _startup(self):
        logger.info("Inisialisasi browser di background...")
        asyncio.create_task(self._initialize_browser_background())

    async def _initialize_browser_background(self):
        for attempt in range(5):
            try:
                logger.info(f"Memulai inisialisasi browser (percobaan {attempt + 1})...")
                await asyncio.wait_for(self._initialize_browser(), timeout=120)
                logger.success("Browser siap!")
                return
            except asyncio.TimeoutError:
                logger.error(f"Inisialisasi timeout (percobaan {attempt + 1})")
            except Exception as e:
                logger.error(f"Inisialisasi gagal (percobaan {attempt + 1}): {type(e).__name__}: {e}")
            try:
                if self.browser:
        ...
        logger.error("Browser gagal diinisialisasi setelah 5 percobaan!")
    ...
    async def _build_page_pool(self):
        ...
        for _ in range(self.thread_count):
            ...
            # Inject stealth script ke setiap context
    ...
    async def get_debug_screenshot(self):
        """Return screenshot debug terakhir."""
        import os, glob
        from fastapi.responses import FileResponse
        debug_dir = "/app/debug_logs"
        files = sorted(glob.glob(f"{debug_dir}/*.png"), reverse=True)
        if not files:
            return JSONResponse(content={"error": "No debug screenshots"}, status_code=404)
        latest = files[0]
        return FileResponse(latest, media_type="image/png", filename=os.path.basename(latest))

    async def process_turnstile_sync(self, url: str = Query(...), sitekey: str = Query(...), action: str = Query(None), cdata: str = Query(None)):
        """Sync endpoint — tunggu sampai token keluar."""
        if self.page_pool.empty():
            return JSONResponse(content={"status": "error", "error": "Server belum siap"}, status_code=503)
        ...
        for _ in range(90):
            ...
            if status not in ("process", None):
                return JSONResponse(content=result, status_code=200 if status != "error" else 422)

        return JSONResponse(content={"status": "error", "error": "Timeout"}, status_code=408)

    async def health(self):
        ready = self.page_pool.qsize() > 0
        return JSONResponse(
            content={"status": "ok" if ready else "initializing", "pages": self.page_pool.qsize()},
            status_code=200
        )
...
if __name__ == "__main__":
    ...
    # Override port dari env variable (Railway/Render inject PORT)
    if os.environ.get("PORT"):
        config["port"] = int(os.environ["PORT"])
        print(f"  ℹ️  Port dari env: {config['port']}")

    # Skip interactive prompt kalau bukan TTY (server/docker)
    if sys.stdin.isatty():
        config = _interactive_config(config)
        _check_xvfb(config.get("headless", True))
    else:
        print("  ℹ️  Mode non-interaktif, langsung pakai config.json")
